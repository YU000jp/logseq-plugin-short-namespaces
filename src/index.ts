...
const pageRefQuerySelectorAll = async (): Promise<void> => {
  ...
  parent.document
    .querySelectorAll(
      'div:is(#main-content-container,#right-sidebar) a[data-ref*="/"]:not([data-orig-text]), div#left-sidebar li[data-ref*="/"] span.page-title:not([data-orig-text]) , div#left-sidebar div.kef-ae-fav-item-name[title]:not([data-orig-text])'
    )
    .forEach((element) => abbreviateNamespace(element as HTMLElement));
  parent.document
    .querySelectorAll(
      'div:is(#main-content-container,#right-sidebar) a.page-ref:not([data-ref*="/"]):not([data-icon])'
    )
    .forEach((element) => SetLinksIconWithoutHierarchy(element as HTMLElement));
...
function observerMainRight() {
  ...
  observer.observe(
    parent.document.getElementById("left-sidebar") as HTMLDivElement,
    {
      subtree: true,
      attributes: true,
      attributeFilter: ["class"],
...
function abbreviateNamespace(namespaceRef: HTMLElement) {
  ...
  if (logseq.settings!.iconMode !== "false" && !namespaceRef.dataset.icon)
    getIcon(namespaceRef, splitText[0] as string);
  const abbreviatedText = abbreviated(
    splitText,
    logseq.settings!.booleanUseDot === true ? ".." : ""
  ) as string;
...
const getIcon = async (namespaceRef, parent: string): Promise<void> => {
  ...
  if (page && page.properties?.icon) {
    ...
    namespaceRef.insertAdjacentHTML(
      "beforebegin",
      page.properties.icon as string
    );
    namespaceRef.dataset.icon = page.properties.icon as string;
  }
};

async function SetLinksIconWithoutHierarchy(
...
const restoreAllNamespaces = () =>
  parent.document
    ...
    .forEach((element) => restoreNamespace(element as HTMLElement));
